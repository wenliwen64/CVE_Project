...
void cal_eff(int cen) {
	...
	for(int trk = 0; trk < NrcTracks; trk++) {
	    ...
	    if(nFitHits<25) continue;  // || nFitHits>50) continue;//modified by Liwen
	    if(ratio<0.52) continue; //|| ratio>1.05) continue;//modified by Liwen
            float dca_cut = 2.0;
/*
            switch(cen){
		case 1:
